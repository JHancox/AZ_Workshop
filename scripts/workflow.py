...
from monai.utils import BundleProperty, ensure_tuple, look_up_option, set_determinism
...
class VistaCell(BundleWorkflow):
    ...
    def __init__(
        ...
        **override,
        ...
        if self.rank == 0:
            ...
            if _log_file is not None: open(_log_file, 'a').close()
        ...
        if seed is not None:
            ...
            logger.info(f"No seed provided, using cudnn.benchmark for performance.")
    ...
    def get_train_dataset_data(self):
        ...
        if isinstance(data_list_files, str):
            data_list_files = ConfigParser.load_config_file(data_list_files) #if it's a path to a separate file with a list of datasets
        else:
        ...
        if not valid_files:
            logger.warn("No validation data found.")
    ...
    def get_train_preprocessing(self):
        ...
        train_xforms.append(mt.ScaleIntensityd(keys="image", minv=0, maxv=1, channel_wise=True))
        train_xforms.append(mt.ScaleIntensityRangePercentilesd(keys="image", lower=1, upper=99, b_min=0.0, b_max=1.0, channel_wise=True, clip=True))
        train_xforms.append(mt.SpatialPadd(keys=["image", "label"], spatial_size=roi_size))
        train_xforms.append(mt.RandSpatialCropd(keys=["image", "label"], roi_size=roi_size)) # crop roi_size (if image is large)
        ...
        train_xforms.append(LabelsToFlows(keys="label", flow_key="flow")) # finally create new key "flows" with 3 channels 1) foreground 2) dx flow 3) dy flow
    ...
    def get_val_preprocessing(self):
        ...
        val_xforms.append(mt.EnsureTyped(keys=["image", "label"], data_type="tensor", dtype=torch.float, allow_missing_keys=True))

        if self.config("prescale", True):
            print('Prescaling val images to 0..1')
            val_xforms.append(mt.ScaleIntensityd(keys="image", minv=0, maxv=1, channel_wise=True))

        val_xforms.append(mt.ScaleIntensityRangePercentilesd(keys="image", lower=1, upper=99, b_min=0.0, b_max=1.0, channel_wise=True, clip=True))
        val_xforms.append(LabelsToFlows(keys="label", flow_key="flow", allow_missing_keys=True))
        
        print("val_preproc_done")
    ...
    def train(self):
        ...
        amp_dtype = {'float32': torch.float32, 'bfloat16': torch.bfloat16, 'float16': torch.float16}[config("amp_dtype")]
        if amp_dtype == torch.bfloat16 and not torch.cuda.is_bf16_supported():
            amp_dtype = torch.float16
            logger.warn(f"bfloat16 dtype is not support on your device, changing to float16, use  --amp_dtype=float16 to set manually")
        ...
        if self.global_rank == 0 and ckpt_path is not None:
            ...
            if mlflow_is_imported:
                if config("mlflow_tracking_uri", None) is not None:
                    ...
                    mlflow.start_run(run_name=mlflow_run_name, log_system_metrics=config("mlflow_log_system_metrics", False))
            ...
            if do_torch_save and ((epoch + 1) % num_epochs_per_saving == 0 or (epoch + 1) >= num_epochs):
                if report_epoch != best_metric_epoch:
                    ...
                    try:
                        ...
                        logger.warn(f"error copying {best_ckpt_path} {intermediate_ckpt_path} {err}")
                        pass
    ...
    def validate(self, validation_files=None, val_key=None, datalist=None):
        ...
        if self.config("pretrained_ckpt_name", None) is None and self.config("pretrained_ckpt_path", None) is None:
            ...
            logger.info(f"Using default model.pt checkpoint for validation.")
        ...
        if len(validation_files) == 0:
            logger.warn(f"No validation files found {datalist} {val_key}!")
            return 0, 0, 0
        ...
        amp_dtype = {'float32': torch.float32, 'bfloat16': torch.bfloat16, 'float16': torch.float16}[self.config("amp_dtype")]
        if amp_dtype == torch.bfloat16 and not torch.cuda.is_bf16_supported():
            amp_dtype = torch.float16
            logger.warn(f"bfloat16 dtype is not support on your device, changing to float16, use  --amp_dtype=float16 to set manually")
    ...
    def infer(self, infer_files=None, infer_key=None, datalist=None):
        ...
        if self.config("pretrained_ckpt_name", None) is None and self.config("pretrained_ckpt_path", None) is None:
            self.parser["pretrained_ckpt_name"] = "model.pt"
            logger.info(f"Using default model.pt checkpoint for inference.")
        ...
        if len(infer_files) == 0:
            logger.warn(f"no file to infer {datalist} {infer_key}.")
            return
    ...
    def val_epoch(
        ...
        if np.any(avg_acc < 0):
            dist.barrier()
            logger.warn(f"Avg accuracy is negative ({avg_acc}), something went wrong!!!!!")
    ...
    def checkpoint_save(self, ckpt: str, model: torch.nn.Module, **kwargs):
        ...
        if self.config("compile", False):
            ...
            state_dict = OrderedDict((k[len("_orig_mod."):] if k.startswith("_orig_mod.") else k, v) for k, v in state_dict.items())
    ...
    def checkpoint_load(self, ckpt: str, model: torch.nn.Module, **kwargs):
        ...
        if not os.path.isfile(ckpt):
            logger.warn("Invalid checkpoint file: " + str(ckpt))
            return
